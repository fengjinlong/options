...
1. **推导财报发布日期**：根据 EPS 条目的季度类型（Q1/Q2/Q3/Q4），映射到实际财报发布月份；若条目包含 `overrideReportDate` 字段，则优先使用该字段指定的具体日期

| 季度 | 默认财报月-日 | 说明 |
|------|-------------|------|
| Q1   | 04-15       | 大多数公司在 4 月中上旬发布 |
| Q2   | 07-15       | 大多数公司在 7 月中上旬发布 |
| Q3   | 10-15       | 大多数公司在 10 月中上旬发布 |
| Q4   | 01-15（次年）| 大多数公司在次年 1 月中上旬发布 |

**支持提前发布覆盖**：若某些公司提前发布财报，可在 EPS 条目中添加 `overrideReportDate` 字段指定实际发布日期，优先于默认月份。例如 TSLA Q1 2026 实际在 4 月 22 日发布：

```json
{ "year": 2026, "quarter": "Q1", "eps": 0.13, "overrideReportDate": "2026-04-22" }
```
...
```json
{
  "NVDA": [
    ...
    { "year": 2024, "quarter": "Q3", "eps": 7.40, "overrideReportDate": "2024-10-22" },
    ...
  ]
}
```

可选字段 `overrideReportDate`：指定该季度 EPS 的实际财报发布日，用于处理提前发布的情况。若不填则使用上表中的默认日期。
...
```json
{
  "新股票代码": [
    ...
    { "year": 2024, "quarter": "Q4", "eps": 1.50, "overrideReportDate": "2025-01-28" }
  ]
}
```

> 注：若季度财报提前发布，可添加 `overrideReportDate` 字段指定实际发布日期，使 TTM PE 计算及时反映新数据。
